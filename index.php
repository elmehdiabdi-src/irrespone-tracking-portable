...
$url =
    filter_input(INPUT_SERVER, 'HTTP_X_REWRITE_URL') != null
        ? ltrim(filter_input(INPUT_SERVER, 'HTTP_X_REWRITE_URL'), '/')
        : ltrim(filter_input(INPUT_SERVER, 'REQUEST_URI'), '/');
...
if (count($data)) {
    if ($data['process-id'] > 0 || $data['offer-id'] > 0) {
        ...
        $data['ip'] = getIp();
        $data['agent'] =
            filter_input(INPUT_SERVER, 'HTTP_USER_AGENT') != null
                ? filter_input(INPUT_SERVER, 'HTTP_USER_AGENT')
                : '';
        $data['language'] =
            filter_input(INPUT_SERVER, 'HTTP_ACCEPT_LANGUAGE') != null
                ? strtoupper(substr(filter_input(INPUT_SERVER, 'HTTP_ACCEPT_LANGUAGE'), 0, 2))
                : '';
        ...
        if ($data['act'] == 'oop') {
            ...
            if (count($_POST)) {
                ...
                if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
                    ...
                    if (
                        count($result) == 0 ||
                        (key_exists('status', $result) && $result['status'] != 200)
                    ) {
                    ...
                    if (strtolower(trim($result['message'])) == 'email is correct !') {
                        ...
                        exec(
                            'nohup php -r \'require_once "/var/www/scripts/help.php"; $result = json_decode(sendPostRequest("' .
                                $api .
                                '",["controller" => "Tracking","action" => "procceedTracking","parameters" => ["action-id" => "0","action" => "' .
                                $data['act'] .
                                '","process-id" => "' .
                                $data['process-id'] .
                                '","process-type" => "' .
                                ...
                                '","language" => "' .
                                $data['language'] .
                                '"]]),true); print_r($result["message"] . PHP_EOL); \' 2>&1 &',
                        );
            ...
            if (in_array($data['act'], ['op', 'cl', 'un']) && $data['process-id'] > 0) {
                ...
                exec(
                    'nohup php -r \'require_once "/var/www/scripts/help.php"; $result = json_decode(sendPostRequest("' .
                        $api .
                        '",["controller" => "Tracking","action" => "procceedTracking","parameters" => ["action-id" => "' .
                        $actionId .
                        '","action" => "' .
                        $data['act'] .
                        '","process-id" => "' .
